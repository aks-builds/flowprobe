  ...
  const flowsHtml = result.flows.map(flow => {
    const stepsHtml = flow.steps.map(step => {
      ...
      const err = step.error
        ? `<div style="color:#dc2626;font-size:12px;margin-top:4px;">${escXml(step.error)}</div>`
        : ''
      return `<div style="padding:8px 12px;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;gap:8px;"><span style="color:${color};font-weight:700;">${icon}</span><span style="font-size:13px;color:#1e293b;">${escXml(step.id)}</span><span style="font-size:11px;color:#94a3b8;margin-left:auto;">${step.durationMs}ms</span>${err}</div>`
    }).join('')
    return `<div style="border:1px solid #e2e8f0;border-radius:8px;margin-bottom:12px;overflow:hidden;"><div style="padding:10px 14px;background:#f8fafc;border-bottom:1px solid #e2e8f0;font-weight:600;font-size:14px;">${escXml(flow.name)}</div>${stepsHtml}</div>`
  }).join('')
